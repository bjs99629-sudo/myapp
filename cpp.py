...
with st.sidebar:
    ...
    st.markdown(f"#### 📢 오늘 ({today_str}) 출석 현황")
    ...
    if not df_all.empty:
        ...
        if not df_today.empty:
            for idx, row in df_today.iterrows():
                ...
                border_color = "#2ecc71" if row["상태"] == "출석" else "#f1c40f"
                st.markdown(f"""
                <div style="padding: 0.6rem; background: rgba(255, 255, 255, 0.03); border-radius: 8px; margin-bottom: 0.5rem; border-left: 4px solid {border_color};">
